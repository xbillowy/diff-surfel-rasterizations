	...
	const bool debug)
{
  ...
  return std::make_tuple(rendered, out_color, out_others, out_weight, radii, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dmeans2D = torch::zeros({P, 4}, means3D.options());  // as return result
  torch::Tensor dL_dmeans2D_abs = torch::zeros({P, 2}, means3D.options());  // as middle result
